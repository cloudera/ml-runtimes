...
RUN apk add --no-cache python-3.14-dev python-3.14-base-dev

RUN ln -s /usr/sbin/python3 /usr/local/bin/python3


COPY etc/pip.conf /etc/pip.conf
COPY requirements/py314/python-comcloud-chainguard-extras-requirements.txt /build/requirements_extras.txt
COPY requirements/py314/python-standard-packages-requirements.txt /build/requirements.txt
...
COPY requirements/py314/pbj-workbench-base-requirements.txt /build/requirements.txt
...
COPY requirements/py314/pbj-jupyterlab-requirements.txt /build/requirements.txt
