...
if copilot_config_dir == "" or not os.path.isfile(copilot_config_dir):
    print(DEFAULT_COPILOT_MODEL)
    exit(0)

copilot_config_file = open(copilot_config_dir)
copilot_models = json.load(copilot_config_file)
copilot_config_file.close()

if copilot_models and "aiInferenceModels" in copilot_models and copilot_models["aiInferenceModels"]:
    for ai_inference_model in copilot_models["aiInferenceModels"]:
        if ai_inference_model["default"]:
            print("cloudera:" + ai_inference_model["name"])
            exit(0)
if copilot_models and "thirdPartyModels" in copilot_models and copilot_models["thirdPartyModels"]:
    for third_party_model in copilot_models["thirdPartyModels"]:
