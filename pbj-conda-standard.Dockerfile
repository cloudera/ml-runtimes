...
COPY requirements/py312/python-standard-packages-requirements.txt /build/requirements.txt
...
COPY requirements/py312/pbj-workbench-base-requirements.txt /build/requirements.txt
...
COPY requirements/py312/pbj-jupyterlab-requirements.txt /build/requirements.txt
...
ENV ML_RUNTIME_KERNEL="Conda" \
    ML_RUNTIME_EDITION="Tech Preview" \
    ML_RUNTIME_DESCRIPTION="Tech Preview edition Jupyterlab Conda runtime provided by Cloudera" \
    CONDA_DIR=/opt/conda
...
RUN \
    ...
    -o /build/miniconda.sh && \
    /bin/bash /build/miniconda.sh -b -p /opt/conda && \
...
ENV CML_JUPYTER_ENSURE_NATIVE_KERNEL="False"

RUN echo 'eval "$(command conda shell.bash hook 2>/dev/null)"' >> /etc/profile.d/02-conda-init.sh
